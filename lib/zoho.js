...
async function fetchAllRecordsUncached(resourceId, worksheetName) {
...
const fetchAllRecordsCached = unstable_cache(
  fetchAllRecordsUncached,
  ["zoho-sheet-records"],
  { revalidate: 60 }
);

export async function fetchAllRecords({ resourceId, worksheetName }) {
  const cacheKey = `${resourceId}:${worksheetName}`;
  ...
  try {
    const all = await fetchAllRecordsCached(resourceId, worksheetName);

    recordsCache.set(cacheKey, {
      records: all,
      expiresAt: now + SHEET_CACHE_TTL_MS,
    });

    return all;
  } catch (error) {
    // Prefer slightly stale data over a hard failure during temporary rate limiting.
